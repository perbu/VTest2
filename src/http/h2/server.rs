...
impl<S: SessionOps> H2Server<S> {
    ...
    /// Perform HTTP/2 connection preface validation and settings exchange
    pub fn accept(&mut self) -> Result<()> {
        if self.connected {
            return Ok(());
        }

        // Receive and validate connection preface (RFC 7540 Section 3.5)
        ...
        while total_read < CONNECTION_PREFACE.len() {
            ...
            if n == 0 {
                return Err(Error::Protocol("Connection closed during preface".to_string()));
            }
            total_read += n;
        }

        if &preface_buf != CONNECTION_PREFACE {
            return Err(Error::Protocol("Invalid connection preface".to_string()));
        }

        // Send initial SETTINGS frame first (before processing client SETTINGS)
        let settings_frame = SettingsFrame::new(self.local_settings.clone());
        self.send_settings(&settings_frame)?;

        // Receive client SETTINGS (this will also send SETTINGS ACK)
        self.recv_settings()?;
                ...
                FrameType::Headers => {
                    ...
                    for (name, value) in decoded {
                        ...
                        match name_str.as_str() {
                            ...
                            ":authority" => request.authority = Some(value_str),
                            ":scheme" => request.scheme = Some(value_str),
                            _ => {
...
mod tests {
    ...
    fn test_request_accessors() {
        ...
        assert_eq!(request.body_string().unwrap(), "{\"test\":true}");
    }
}

...
impl<S: SessionOps> H2Server<S> {
    ...
    /// Accept HTTP/2 connection (validate preface and exchange settings)
    pub fn accept(&mut self) -> Result<()> {
        if self.preface_received {
            return Ok(());
        }

        // Read connection preface from client (RFC 7540 Section 3.5)
        let mut preface_buf = vec![0u8; CONNECTION_PREFACE.len()];
        let mut total_read = 0;

        while total_read < CONNECTION_PREFACE.len() {
            let n = self.session.read(&mut preface_buf[total_read..])?;
            if n == 0 {
                return Err(Error::Protocol("Connection closed before preface".to_string()));
            }
            total_read += n;
        }
        ...
        if &preface_buf[..] != CONNECTION_PREFACE {
            return Err(Error::Protocol(format!(
                "Invalid connection preface: expected {:?}, got {:?}",
                CONNECTION_PREFACE, &preface_buf[..]
            )));
        }
    ...
    pub fn recv_request(&mut self) -> Result<H2Request> {
        ...
        while !stream_ended {
            ...
            match frame_type {
                FrameType::Headers => {
                    if stream_id == CONNECTION_STREAM_ID {
                        return Err(Error::Protocol(
                            "HEADERS frame cannot have stream ID 0".to_string(),
                        ));
                    }
                ...
                FrameType::Data => {
                    if !headers_received {
                        return Err(Error::Protocol(
                            "DATA frame received before HEADERS".to_string(),
                        ));
                    }

                    if stream_id != current_stream_id {
                        // Interleaved DATA frame for different stream - skip for now
                        continue;
                    }
        ...
        if !headers_received {
            return Err(Error::Protocol("No HEADERS frame received".to_string()));
        }
    ...
    pub fn new() -> Self {
        H2ServerBuilder {
            settings: SettingsBuilder::new()
                ...
                .enable_push(true) // Server can push
...
mod tests {
    ...
    fn test_request_accessors() {
        ...
        assert_eq!(request.body_string().unwrap(), "test");
    }
}
